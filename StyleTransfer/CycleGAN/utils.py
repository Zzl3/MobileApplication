...
def train_G(Da, Db, B_fake, loss, A_fake, l1_loss, A_rec, A_real, B_rec, B_real, Ga, Gb):
    ...
    Ga_gan_loss = loss(Da_out_fake, torch.ones(Da_out_fake.size()).cpu())
    Db_out_fake = Db(A_fake)
    #Gb_gan_loss = loss(Db_out_fake, torch.ones(Db_out_fake.size()).cuda())
    Gb_gan_loss = loss(Db_out_fake, torch.ones(Db_out_fake.size()).cpu())
...
def train_D(Da, Db, B_fake, B_real, loss, A_fake, A_real):
    ...
    ls_Da1 = loss(Da_fake_out, torch.zeros(Da_fake_out.size()).cpu())
    #ls_Da2 = loss(Da_real_out, torch.ones(Da_real_out.size()).cuda())
    ls_Da2 = loss(Da_real_out, torch.ones(Da_real_out.size()).cpu())
    ...
    ls_Db1 = loss(Db_fake_out, torch.zeros(Db_fake_out.size()).cpu())
    #ls_Db2 = loss(Db_real_out, torch.ones(Db_real_out.size()).cuda())
    ls_Db2 = loss(Db_real_out, torch.ones(Db_real_out.size()).cpu())
